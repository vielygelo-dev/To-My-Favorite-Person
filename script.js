...
/* ---------- 8. SONG PLAYER: play/pause, fake visualizer, progress bar, autoplay ---------- */
function initSongPlayer() {
  ...
  // The <audio> tag has preload="none" for fast initial load; switch it to
  // "auto" so the browser actually fetches the file for autoplay.
  audio.preload = 'auto';
  ...
  // Most browsers block audio with sound from autoplaying until the visitor
  // has interacted with the page at least once. We try immediately; if that
  // fails (it usually will, the first time), we fall back to starting
  // playback on the visitor's very first click/tap/keypress anywhere on
  // the page, so the song still starts almost right away without them
  // needing to find the play button.
  const tryAutoplay = () => audio.play().catch(() => false);
